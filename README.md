...
if game:GetService("CoreGui"):FindFirstChild("Kaitun Hub🐓") then
	game:GetService("CoreGui"):FindFirstChild("Kaitun Hub🐓"):Destroy()
end
...
local serv = win:Server(" Cute Hub💗 (TH) | Kaitun 🐓","")

local AutoFarm = serv:Channel("AutoFarm✨ (ออโต้ฟาม)", "")
local stats = serv:Channel("stats🍙(สเตตัส)","")
local Setting = serv:Channel("Setting💖 (การตั้งค่า)", "")
...
AutoFarm:Toggle("Auto SetSpawn Point💤 (ออโต้เซ็ตจุดเกิด)",nil,function(x)
    _G.Set = x
end)
...
local SelectWeapon = AutoFarm:Dropdown("Select Weapon🌹 (เลือกของที่จะใช้ฟาม)",Wapon,function(Value)
    SelectToolWeapon = Value
    SelectToolWeaponOld = Value
end)

AutoFarm:Button("Refresh Weapon💫 (รีUiหากไม่ขึ้นของในตัว)",function()
	SelectWeapon:Clear()
	for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do  
		if v:IsA("Tool") then
...
AutoFarm:Button("God🔥 fast attack (สุดยอดการโจมตี)",function()
...
AutoFarm:Button("Redeem All code🎁(กดเพื่อใส่ทุกโค้ด)",function()
    function Code(Text)
